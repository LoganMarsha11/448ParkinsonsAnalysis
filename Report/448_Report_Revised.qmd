...
Grid search results for the nondominant foot showed that the gait SVM model produced a balanced fit with strong predictive performance, achieving results similar to random forest and second only to Ridge regression. In contrast, the TUG SVM model showed extreme overfitting, with near-perfect  $R^2_{Train, Adj}$ and substantially high test RMSE, and would not be considered a reliable predictive model.
