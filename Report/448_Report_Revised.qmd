...
csl: https://github.com/citation-style-language/styles/raw/master/apa.csl
---
...
Data was collected from 85 adults without Parkinson’s disease, approximately three-quarters of whom were classified as older adults ($\ge 65$ years). Each participant wore foot-mounted sensors equipped with 3-axis inertial measurement units (IMUs), generating six signals in total (three gyroscope axes and three accelerometer axes). The sensor used for both accelerometer and gyroscope measurements was a Trigno wireless biofeedback system. [@fig-axes] illustrates the positive foot-tapping direction for each axis and provides an example of four sensors positioned within the Trigno system’s module slots used for data transmission [@noroozi5297926detecting] [@trigno]. From these measurements, 179 engineered features were extracted in MATLAB, capturing time- and frequency-domain characteristics in each plane, as well as magnitude-based metrics.

```{r, echo=FALSE, results='asis'}
#| label: fig-axes
#| fig-cap: "Positive IMU axis directions and example Trigno sensor placement."
#| out-width: "75%"
##| out-height: "100%"
library(knitr)
include_graphics("trigno_axes.jpeg")
#May have to adjust figure size
```

Although many of these engineered features are highly correlated, our primary objective is predictive accuracy. While reducing multicollinearity is beneficial, our focus is on minimizing root mean squared error (RMSE) to best assess predictive performance. RMSE is a common regression metric defined as the square root of the average squared difference between predicted and observed values, providing a direct measure of a model’s typical prediction error [@james2013introduction]. Multiple modelling approaches are explored in this project, including filter- and wrapper-based feature selection methods, ensemble learning, regularization techniques, kernel methods, and dimensionality reduction strategies, in order to identify the optimal feature subset and model for predicting gait time and TUG time.

## Methods

#### Data Cleaning and Preprocessing

In this dataset, the number at the end of each feature name indicates the axis along which the signal was measured. Axis 1 corresponds to the X-axis (left–right), with positive values indicating rightward movement; Axis 2 corresponds to the Y-axis (front–back), with positive values indicating forward movement; and Axis 3 corresponds to the Z-axis (up–down), where positive values reflect upward movement. Axis 4 refers to the vector magnitude, which is calculated only for accelerometer features and represents the combined acceleration across all three spatial axes. Features beginning with acc- are derived from the accelerometer, whereas those beginning with gyro- come from the gyroscope. Time-domain features describe how the signal changes over time, while time–frequency features characterize how the signal’s energy is distributed across both time and frequency, similar to what is observed in a spectrogram. 

Some features were derived by summing components from decomposed wavelength signals using the Hilbert–Huang Transform [@liu2020hilbert]. Additional features were extracted from Continuous Wavelet Transform (CWT) scalograms [@bobic2018classification]. More detailed descriptions of the features can be found in the feature description file included in the GitHub repository.
...
```{r, echo=FALSE, results='asis'}
#| label: fig-schematic
#| fig-cap: "Schematic of the nondominant foot stepwise regression workflow for predicting Gait Time and TUG Time"
#| out-width: "100%"
##| out-height: "100%"
include_graphics("Schematic_Stepwise1205.jpeg")
#May have to adjust figure size
```

Merging dominant and nondominant foot features into a single model, as well as modeling the dominant foot separately, did not improve predictive performance. Both approaches produced consistently higher RMSE values than the nondominant only models. As a result, the analyses in this report focus solely on models trained for the nondominant foot.

#### Stepwise

Applied forward selection and mixed stepwise regression on the pre-filtered dataset using the `step()` function from R’s **stats** package, with Akaike Information Criterion (AIC) as the selection criterion [@stats]. The model search explored a range from an intercept-only model to a full model containing all predictor variables. Due to the dataset’s high dimensionality, more features than observations despite filtering highly correlated variables, methods like backward elimination and recursive feature elimination could not be computed. Forward and mixed selection, which start from a smaller model, were able to handle this scenario without issue.
...
#| label: tbl-1
#| tbl-cap: "Nondominant Foot Gait Time Model Summaries (Bold Indicates Lowest Test RMSE)"
library(kableExtra)
...
After the correlation filter, 68 features remained for the nondominant foot. Forward and mixed stepwise produced highly similar models. Mixed stepwise selected 20 features and forward selected 17. Both models included a mixture of gyroscope and accelerometer domain features, along with age. Mixed stepwise produced lower training RMSE and higher adjusted R-squared ($R^2_{Train, Adj}$), whereas forward selection yielded the lower test RMSE. Both models showed noticeably higher test RMSE than training RMSE.

\FloatBarrier

#### Lasso

Lasso performed very effectively in reducing the dimensionality of the dataset, selecting only 5 features for nondominant foot out of the original 136 predictors at optimal $\lambda$ = 0.2026074. Predictive performance was strong, with test RMSE among the best observed across all models. To further assess the impact of variable selection, the selected features from lasso were fitted in a standard linear model. The training fit improved slightly, while the test RMSE increased only marginally (+0.022 seconds). A deeper examination of the summary table for the linear model ([@tbl-2]) shows that only one feature, the standard deviation of the intertap interval from the accelerometer (`acctime_ITIsd`), was statistically significant at the $\alpha = 0.05$ level, along with the intercept. Four of the five selected predictors were derived from accelerometer features, with the remaining one being participant age. The overall model $p$-value was extremely small (8.996e-05).
...
Ridge regression produced the lowest test RMSE for gait time, using optimal $\lambda$ = 14.97415. Ridge retained all 136 predictors and produced stable predictions.

#### Elastic Net

The average RMSE for all $\alpha$ values were very similar. However it was identified that 0 was the optimal value. For the fixed seed retrain, I decided to use 0.1 instead of 0 since ridge regression is already represented in our results table, as this was the alpha with the second lowest RMSE and a better representative of elastic net regularization. The elastic net simulation performed similarly to lasso but did not perform better than ridge regression in terms of test set RMSE.

#### Random Forest

Random forest produced a weak training fit but a competitive test RMSE, only slightly behind ridge. A small range of $m$ values was examined, but results remained consistent and did not surpass ridge. Therefore, the default number of predictors considered at each split was retained. In the variable importance plot for the nondominant foot ([@fig-2]), the top three predictors in terms of percent increase in mean squared error were the minimum signal and variance of the z-axis accelerometer (`acctime_Minsignal_3`, `acctime_Varsignal_3`) and the standard deviation of summed foot-tapping energy in the y-axis (`accTimeFrequency_SDSumenergy_2`). Feature importance values decreased notably after these top three predictors. All of the top ten predictors (%IncMSE) were derived from accelerometer features.
...
Cross-validation selected 118 trees for the nondominant foot, which was then used for predictions. Boosting achieved the second lowest test error among the ensemble methods, performing slightly worse than random forest but better than bagging.

\FloatBarrier

#### Support Vector Machines

From the grid search, $\gamma$ = 0.001 and C = 1 produced the lowest test RMSE among all combinations evaluated. The resulting test RMSE was identical to that of random forest, the ensemble method with the lowest test error.

#### Principal Component Analysis

The Kaiser criterion retained 22 components, while the scree plot showed no clear elbow point but suggested roughly 2–3 components. To capture a substantial portion of the variance, 12 components were selected, which explained approximately 80% of the variance in the training data. These 12 principal components were fitted in a linear model predicting gait time. Only two components were statistically significant, and predictions were noticeably less accurate than other tested models.

#### Partial Least Squares Regression

Seven components were required to capture roughly 80% of the variance. The PLSR models exhibited clear overfitting, with one of the lowest training RMSE values among all methods but one of the highest test RMSE values.

#### Bland-Altman

For gait time, the Bland–Altman plot for one of the best-performing models (the LASSO-fitted model) is shown in [@fig-3]. The mean bias is slightly below zero, and all observations fall within the 95% limits of agreement. The spread of the differences increases as the average gait time increases, with points appearing more dispersed at higher values. The scale-location plot in [@fig-4] similarly shows a mild upward trend in the LOESS curve (approximately 0.5 to 1), which aligns with the Bland–Altman indication of non-constant variance; however, this increase is small and not practically substantial.
...
To summarize the gait results, ridge regression achieved the lowest test RMSE, making it the best-performing predictive model. However, the lasso-selected linear model produced a very similar RMSE while using far fewer predictors, offering a substantially simpler alternative. Further interpretation of these findings is provided in the Discussion section.
...
```{r, echo=FALSE, results='asis'}
#| label: tbl-3
#| tbl-cap: "Nondominant Foot TUG Time Model Summaries (Bold Indicates Lowest Test RMSE)"

tug_df <- data.frame(
  Model = c("Lasso", "Lasso Fitted", "Ridge", "Elastic Net, $\\alpha = 0.1$", "Forward Selection", "Mixed Stepwise", "Random Forest", "Bagging", "Boosting", "Support Vector Machine", "PCR", "PLSR"),
  `Number of Features Selected` = c(24, 24, 136, 51, 13, 13, 136, 136, 136, 136, 136, 136),
  R2_Train = c(0.6909447, 0.6442, 0.5507235, 0.646077, 0.6718, 0.6718, 0.2678, 0.2614, 0.7153307, 0.9902096, 0.4666, NA),
  RMSE_Train = c(2.135414, 1.732116, 2.574666, 2.28517, 1.928428, 1.928428, 3.286923, 3.301226, 2.049436, 0.3800714, 2.486625, 1.658824),
  RMSE_Test = c(3.261276, 3.502438, 2.653579, 2.952905, 3.811006, 3.811006, 2.390737, 2.570422,  2.52918, 2.699941, 2.94344, 2.86619)
) #Table 5
...
After correlation filtering, 61 features were retained. Both forward and mixed stepwise approaches selected identical models, with 13 features. The selected features included a mixture of accelerometer and gyroscope variables, along with age. Despite this reduction in dimensionality, the stepwise models produced the highest test RMSE among all methods.

#### Lasso

Lasso effectively reduced the dimensionality of the dataset, selecting 24 features at best $\lambda$ = 0.2707325 out of the original 136 predictors. While the model demonstrated a reasonable fit to the training set, its test RMSE was higher than several ensemble methods. Refitting the nonzero coefficients in a standard linear model did not improve performance; the test RMSE increased relative to the lasso model. As the refitted linear model did not provide performance gains, no further examination of its coefficient estimates was pursued.

#### Ridge

The ridge model showed improvement in results compared to lasso, delivering solid predictive performance on the test set. However, its test RMSE remained slightly higher than those achieved by the ensemble approaches. This model was obtained at $\lambda$ = 23.00559.

#### Elastic Net

The nondominant foot achieved its best performance at $\alpha$ = 0. For consistency with the fixed-seed retraining procedure used in the gait analysis, the model was retrained with $\alpha$ = 0.1, yielding similar performance. The elastic net model achieved a lower test RMSE than lasso but remained less accurate than the ensemble methods.

\FloatBarrier

#### Random Forest

Random forest achieved the lowest test RMSE across all models, making it one of the strongest predictive approaches for TUG time. Across the range of $m$ values tested, $m=50$ produced the lowest test RMSE and was retained for interpretation. As shown in [@fig-5], Age was by far the most influential predictor based on percent increase in mean squared error, followed by the minimum and variance of the z-axis accelerometer signal (`acctime_Minsignal_3`, `acctime_Varsignal_3`). Log-transformed power features from the z-axis and magnitude (`acctime_Powerlog_3`, `acctime_Powerlog_4`) also ranked highly based on percent increase in Mean Squared Error. Gyroscope-derived features were also more prominent in the TUG models compared to the gait random forest models. 
...
tugRF <- randomForest(
  ...
  mtry = 50, #total predictors minus response and excluded var
  importance = TRUE
)
#tugRF
...
In summary of the TUG results, the ensemble methods (random forest, bagging, and boosting) achieved the lowest test RMSE values overall. Among them, random forest produced the strongest predictive performance and is considered the best-performing model for TUG time.
...
Our results were consistent with this pattern and with findings from similar foot-tapping studies [@hinman2019validity]. Although not statistically significant, the dominant foot showed a slightly higher average number of taps within the fixed time interval compared to the nondominant foot.

This small performance difference may also relate to the predictive accuracy gap observed between the dominant and nondominant models. While not tested directly in this analysis, greater variability in dominant foot response patterns, such as slightly higher tap counts and additional fluctuations in other features could make the dominant foot more difficult to model, contributing to its slightly higher test RMSE.

Focusing now on the nondominant foot findings, the additional features in the mixed stepwise model slightly improved variance explained, reducing training RMSE and increasing $R^2_{Train, Adj}$. However, forward selection produced a lower test RMSE, indicating better generalization to unseen data. Overall, stepwise regression provided a useful balance between interpretability and predictive performance in this high-dimensional setting. Both variants still showed evidence of overfitting, with low training RMSE but higher test RMSE, suggesting that stepwise procedures may offer limited generalizability for predicting gait and TUG time.

For the gait model, lasso regression performed well. The nearly identical test RMSE between the original lasso model and the refitted linear model indicates that the simplified linear version preserved predictive accuracy while enabling full statistical inference. Notably, all selected features came from the accelerometer (plus age), suggesting that gait time can be predicted effectively without gyroscope-derived inputs. The very small overall model $p$-value confirms that the fitted linear model is statistically significant. Although lasso did not produce the lowest overall RMSE, the difference was small enough that the gain in model simplicity outweighed the slight loss in predictive performance. In contrast, the TUG lasso model showed weaker predictive performance. When the nonzero coefficients were refitted into a standard linear model, test RMSE increased more noticeably, indicating that the shrinkage imposed by lasso was essential for stabilizing predictions.
...
Elastic Net closely mirrored the lasso model for gait. For TUG and gait measures, the elastic net models did not surpass ridge regression in terms of test RMSE, as simulations indicated that $\alpha$ = 0 (equivalent to ridge) produced the most accurate model. Nonetheless, elastic net offered a modest improvement over lasso in predicting TUG time, highlighting its ability to balance sparsity with predictive consistency.
...
Grid search results showed that the gait SVM model produced a balanced fit with strong predictive performance, achieving results similar to random forest and second only to Ridge regression. In contrast, the TUG SVM model showed extreme overfitting, with near-perfect  $R^2_{Train, Adj}$ and substantially high test RMSE, and would not be considered a reliable predictive model.
...
However, this behaviour was not substantial. The Scale–Location plot did not provide strong evidence of heteroskedasticity. Although the LOESS curve showed slight curvature and a mild increase in residual spread across fitted values, the change was small and unlikely to affect model validity. Overall, the assumption of constant variance was considered acceptable.

In contrast, the Bland–Altman plot for TUG time using the random forest model showed minimal overprediction, with relatively stable differences across the range of averages. Although two observations fell outside the limits of agreement, the overall error structure appeared more uniform than in the gait time model.

## Conclusion

For gait time, the nondominant foot lasso-selected linear model provided the best balance between interpretability and predictive performance. It achieved a test RMSE of approximately 0.795 seconds, indicating strong accuracy while using only 5 predictors, all accelerometer-derived features plus age, compared to the 136 required by ridge regression. Although ridge achieved the lowest test RMSE overall, its marginal improvement in accuracy did not outweigh the major gains in simplicity and interpretability provided by the lasso model. Consequently, the lasso-based linear model is more practical and better suited for clinical or applied settings.

For TUG time, the nondominant foot random forest model delivered the strongest predictive performance on unseen data, with a test RMSE of roughly 2.391 seconds. While this value appears larger than the gait time RMSE, it is proportional to the substantially longer duration of the TUG test, making the model’s accuracy reasonable in context. Although random forest does not provide interpretable coefficients or $p$-values, its variable importance rankings identified the most influential features, providing meaningful insight into movement characteristics associated with TUG performance. Despite its relatively low $R^2_{Train, Adj}$, our primary objective was prediction rather than inference, making test RMSE the appropriate criterion; under this metric, random forest was clearly the best-performing model. These results indicate that accelerometer-derived features, particularly those capturing time-domain dynamics, are more informative for predicting TUG performance than gyroscope or frequency-based measures.

Together, these findings highlight the complementary value of a sparse linear model derived from lasso-selected features and an ensemble-based approach for predicting functional mobility outcomes from high-dimensional wearable-sensor data.
