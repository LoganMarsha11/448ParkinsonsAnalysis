...
Used Partial Least Squares Regression (PLSR) to predict gait and TUG time using the **pls** package with the orthogonal scores algorithm `(‘oscorespls’)`, ensuring stable computation of components under high collinearity, scaling all predictors and excluding the response variables. Unlike PCR, PLSR identifies components that simultaneously capture variance in the predictors and maximize covariance with the response, allowing for a more efficient dimensionality reduction [@boulesteix2007partial].
...
*Results for the gait time models are presented in Table 1 (dominant foot) and Table 3 (nondominant foot).*
...
Random forest produced moderate training fit but competitive test RMSE, slightly behind Ridge for the nondominant foot. Some noticeably influential variables (in terms of percent increase in Mean Squared Error) for dominant foot in the importance plot were coefficient of variation and standard deviation of the intertap interval features (acctime_ITIcv, acctime_ITIsd), Age , Standard deviation of summed foot-tapping energy over time in the z-axis (accTimeFrequency_SDSumenergy_3), and Kurtosis signal in the x-axis (acctime_Kurtosissignal_1). Three stand out important features for the nondominant foot (shown in figure 3) is the minimum and variance signal in the z-axis (acctime_Minsignal_3, acctime_Varsignal_3) as well as standard deviation of summed foot-tapping energy over time in the y-axis (accTimeFrequency_SDSumenergy_2). All top ten features (in terms of %IncMSE) in figure 1 and 3 were extracted from the accelerometer. 

*Bagging*

The model’s performance on the test set was very similar to that of the random forest, showing limited improvement and overall weaker predictive accuracy compared to ridge regression. The variable importance plot also revealed many of the same influential features as those identified by the random forest model.
...
For gait time, the Bland–Altman plot for one of the best-performing models (the nondominant LASSO-fitted model) is shown in Figure 5. The mean bias is slightly below zero, and all observations fall within the 95% limits of agreement. The spread of the differences increases as the average gait time increases, with points appearing more dispersed at higher values.

## TUG Results

*Results for the TUG time models are presented in Table 2 (dominant foot) and Table 4 (nondominant foot).*
...
#Use $...$ math and \\mathrm so no extra package is required
kbl(
  gait_df_display,
...
```{r, fig.width=10, fig.height=8, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Top 10 Influential Variables in Dominant Gait Time Prediction Using Random Forest"}
...
varImpPlot(gaitRF, n.var = 10, main="") #Figure 1
```
...
```{r, fig.width=10, fig.height=8,echo=FALSE, fig.cap="Top 10 Influential Variables in Dominant TUG Time Prediction Using Random Forest"}
set.seed(101)
...
varImpPlot(tugRF, n.var = 10, main="") #Figure 2
...
```{r, fig.width=10, fig.height=8, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Top 10 Influential Variables in Nondominant Gait Time Prediction Using Random Forest"}
set.seed(101)
...
varImpPlot(gaitRF, n.var = 10, main="") #Figure 3
```
...
```{r, fig.width=10, fig.height=8,echo=FALSE, fig.cap="Top 10 Influential Variables in Nondominant TUG Time Prediction Using Random Forest"}
set.seed(101)
...
varImpPlot(tugRF, n.var = 10, main="") #Figure 4
```

```{r, fig.cap="Bland–Altman Plot for Nondominant Foot Gait Time (LASSO-Selected Linear Model)", echo=FALSE, warning=FALSE, message=FALSE}
library(ggplot2)
#Lasso Fitted
...
```{r, fig.cap="Bland–Altman Plot for Nondominant Foot TUG Time (Random Forest Model)", echo=FALSE}
#Random Forest
...
ggplot(BAdf, aes(x = avg, y = diff)) +
  ...
  theme_minimal(base_size = 14) #figure 6
```
...
```{r, echo=FALSE, fig.cap="Scale–Location Plot for the LASSO-Selected Linear Model of Nondominant Foot Gait Time"}
#fig.width=10, fig.height=8
plot(gait_lasreg, which=3, sub="") #Figure 7
```
